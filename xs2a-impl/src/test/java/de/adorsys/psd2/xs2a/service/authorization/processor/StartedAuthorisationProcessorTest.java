...
class StartedAuthorisationProcessorTest {

    @InjectMocks
    private StartedAuthorisationProcessor processor;
    ...
    void execute() {
        ...
        when(processorService.doScaStarted(request)).thenReturn(new AuthorisationProcessorResponse());

        processor.execute(request, processorService);

        verify(processorService, times(1)).doScaStarted(request);
    }
}
