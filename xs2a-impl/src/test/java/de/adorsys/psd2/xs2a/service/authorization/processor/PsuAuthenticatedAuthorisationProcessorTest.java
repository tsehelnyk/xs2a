...
class PsuAuthenticatedAuthorisationProcessorTest {

    @InjectMocks
    private PsuAuthenticatedAuthorisationProcessor processor;
    ...
    @Test
    void scaStatus() {
        assertEquals(Xs2aScaStatus.PSUAUTHENTICATED, processor.getScaStatus());
    }

    @Test
    void execute() {
        AisAuthorisationProcessorRequest request = new AisAuthorisationProcessorRequest(null, null, null, null);
        when(processorService.doScaPsuAuthenticated(request)).thenReturn(new AuthorisationProcessorResponse());

        processor.execute(request, processorService);

        verify(processorService, times(1)).doScaPsuAuthenticated(request);
    }
}
